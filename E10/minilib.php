...
function getRandomWordCatMask($array, $min, $max) {
    $random = rand($min, $max);
    $counter = 0;
    $flag = false;
    $word = null;
    ...
    $mask = preg_replace('/[A-Z]/', '_', $word);
    return $flag ? $word.'-'.$category.'-'.$mask : false;
}

function getLetter($id) {
    $array = range('A', 'Z');
    preg_match('/\d+/', $id, $letter);
    $letter = (int)$letter[0] - 1;
    return $array[$letter];
}